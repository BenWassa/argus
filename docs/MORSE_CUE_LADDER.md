...
**#48 note.** Learn now runs a guided lesson with its own formative support
ladder (`taught / cued / solo / settled`, in `Topic.lessonProgress`). That is a
different ladder in a different field answering a different question, and it is
documented in `docs/MORSE_LESSON.md`. Nothing on this page changed: the five
Test rungs, their identifiers, `CUE_FADE_STREAK`, the payload boundary and the
retention separation are all exactly as they were. A Learn answer cannot write
`ItemCueEvidence` at all.
...
`src/lib/distractors.ts` (evidence-informed alternatives) over
`src/lib/confusion.ts` (the shared confusion model),
...
#48 keeps that architecture too. It makes Learn's first exposure a guided lesson
rather than a page to scroll, and gives that lesson its own formative retrieval,
but the *scored* ladder stays here and Learn still writes no cue state.
...
#48 moved that model, unchanged, into `src/lib/confusion.ts`, so character
ordering, Test distractors and the Learn lesson's alternatives share one
definition instead of three. `distractors.ts` re-exports it, and the module is
deliberately dependency-free — no learner state, no cue ladder, no scheduler —
so the Learn lesson can use it without acquiring the ability to write evidence.
...
content. Since #48 the profile's `mnemonicId` and `textLabel` are likewise
*derived* from the canonical letter, because the visual grammar is a pure
function of it; a topic whose Learn content carries a `morse-character-packet`
still overrides them. That is what let #48 take the thirteen packet sections out
of the seeded topic without removing the Test SVG cue, and there is a test
asserting the derived values for all 26. The verbal set itself is generated by
letter from the versioned Argus table and is not durable learner state.
...
- No cue state is written by Learn, and no Learn surface reads one. #48's
  lesson is a separate ladder in a separate field; `src/lib/morseLesson.ts` does
  not import this module, and a test asserts it.
