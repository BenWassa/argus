...
**Offline-first hardening is active scope, not yet a shipped guarantee.** Issue #113 and `docs/open/ISSUE_113_OFFLINE_FIRST_RUNTIME.md` own the next runtime/storage step: replace the current localStorage persistence boundary with IndexedDB, make local mutation plus sync intent atomic, add a durable retry/outbox path, guarantee production cold-launch through build-derived app-shell caching, define provisioned-device offline auth behavior, and make storage durability visible. Until #113 lands, the local-first interaction model above should not be read as proof that every production cold-launch/update/retry failure mode is already covered.
