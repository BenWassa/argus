...
The library is **local-first and synced**, in that order. Once an authenticated account has completed startup recovery, the UID-bound browser cache is the copy Argus reads and writes for interaction. Every learner mutation is durable locally before cloud synchronization, so established accounts continue working offline or on a flight. Cloud writes are coalesced; failed writes remain pending and retry with bounded backoff and on reconnect.

A Firebase-configured build asks who you are before any learner surface mounts. After Firebase identifies a UID, `Today`, `Library` and learning surfaces remain behind the gate until Argus has resolved that UID's local/cloud recovery matrix. A returning owner sees a brief "Checking your session…" while both identity and recoverable learner state are resolved. A build with no Firebase configuration has nothing to gate and remains entirely local.

Startup recovery is conservative. Missing local state restores from valid cloud state. Corrupt local bytes are preserved in a UID-scoped recovery store before valid cloud state is restored. A validated existing local library seeds an empty cloud account on first authenticated migration. Fresh state is created only after both local and cloud are confirmed absent. If an established UID cache exists and cloud is temporarily unavailable, Argus opens that validated cache offline. If first binding has only legacy/fresh state and cloud cannot be checked, the gate stays closed rather than risking a fresh/default overwrite of recoverable remote progress.

Sync carries the exact parser-accepted v5 topic JSON rather than inventing a second Firebase progress schema. That includes durable scheduler/history/evidence/acquisition fields and Morse `lessonSitting`. Arriving cloud records go through the same parser and catalog-reconciliation boundary used by storage/import before they can become active learner state. Library-level catalog-delivery metadata is synchronized separately because it belongs to no single topic.

Each Google UID has its own local cache, sync ledger, pending-write marker and Firestore subtree. The former shared `argus.library.v5` record is a one-time migration source only. Signing out preserves that UID's recoverable cache/cloud copy but removes the active learner library from memory, so another account can never see or inherit it during account switching.

Where a topic genuinely changed on two devices at once, **neither copy is overwritten**. Argus does not use timestamp last-write-wins. The planner reports incompatible concurrent edits; it also treats edit/delete races as conflicts and refuses a remote copy that would drop attempts/history or item evidence already held locally. Firestore topic writes use monotonic revisions and transactions so a safe planner decision cannot be defeated by a race between reading the snapshot and writing the document.

This is the documented first-release conflict position. Field-level merge remains a possible later refinement, not a prerequisite for safe backup. The exact implementation/recovery matrix and physical-device acceptance checklist are recorded in `docs/closed/ISSUE_93_FIREBASE_PROGRESS_SYNC.md`; `docs/open/PROGRESS_ARCHITECTURE.md` remains authority for what learner progress means.

Access is owner-only. Security Rules identify the owner by their verified Google address — every spelling one account may present, such as the `gmail.com`/`googlemail.com` pair, so a provider detail can never lock the real owner out — and key every path by their UID, so being signed in to Google is not authorization. No second account has a read or write path into the project. Topic creates begin at revision 1 and topic updates must advance exactly one revision. See `docs/open/CONTENT_INBOX.md` and `firestore.rules.template`.
